...
# gh_audit_v3_patched.sh — hygiene flags detection FIXED vs v3
# v3->v3p: awk changed from $6=="NONE" to $6=="" (empty string matches GitHub's actual output)
# Same retry/backoff/pacing/ERR-trap protections as v3.
# [canary] gh_audit_v3_patched_CANARY_MARKER
set -euo pipefail
...
say(){ printf '[%s] %s\n' "$1" "$2"; }
ghr(){ local a; for a in 1 2 3; do if "$@"; then return 0; fi; say held "gh attempt $a failed"; sleep $(( a * 10 )); done; return 1; }
...
ghr gh repo list "$OWNER" --limit 200 \
  ...
  --jq '.[] | [.name, (.defaultBranchRef.name // "-"), (.isArchived|tostring), .updatedAt, (.description // ""), (.licenseInfo.spdxId // "")] | @tsv' \
  > "$BASE/repo_inventory.tsv"
say banked "inventory: $(wc -l < "$BASE/repo_inventory.tsv") repos"
awk -F'\t' '$6==""{print $1"|NO_LICENSE"} $5==""{print $1"|NO_DESC"}' "$BASE/repo_inventory.tsv" >> "$BASE/hygiene_flags.tsv" || true
...
while IFS=$'\t' read -r REPO DB ARCH UPD DESC LIC; do
  ...
  while read -r BR; do
    ...
    if AHEAD=$(ghr gh api "repos/$OWNER/$REPO/compare/$DB...$BR" -q '.ahead_by' 2>/dev/null); then
      if [ "${AHEAD:-0}" -gt 0 ]; then printf '%s|%s|+%s\n' "$REPO" "$BR" "$AHEAD" >> "$BASE/unmerged_ahead.tsv"
      else printf '%s|%s\n' "$REPO" "$BR" >> "$BASE/identical_branches.tsv"; fi
    else printf '%s|%s|NO_COMMON_ANCESTOR\n' "$REPO" "$BR" >> "$BASE/unmerged_ahead.tsv"; fi
  done < <(gh api "repos/$OWNER/$REPO/branches" --paginate -q '.[].name' 2>/dev/null)
  ghr gh pr list -R "$OWNER/$REPO" --state closed --limit 100 --json number,title,closedAt,headRefName,state \
