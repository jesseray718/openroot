#!/bin/bash
# write_context_bridge.sh — session close-out bridge, sealed with SHA256
set -eu
TS=$(date +%Y%m%d_%H%M%S)
DIR=/home/jesse/openroot/context_bridge
SEED=$DIR/session-$TS.md
mkdir -p "$DIR"

cat > "$SEED" << 'SEED_EOF'
# CONTEXT BRIDGE — OpenRoot Session Seed
author: Jesse McMillen Ray (jesse, OpenRoot) | node: optiplex3060 | written: SESSION_CLOSE
...
H=$(sha256sum "$SEED" | awk '{print $1}')
echo "$H  $SEED" >> /home/jesse/openroot/context_bridge/seed_master.log
echo "[sealed] $SEED"
echo "  sha256 $H"
echo "[gate] paste to next window: cat $SEED"
